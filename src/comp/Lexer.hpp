...
struct Positiion {
  Positiion(size_t line, size_t offset)
      : line(std::move(line)), offset(std::move(offset)) {}
...
enum ScanMode { PLAIN, DOUBLE_QUOTED, TRIPLE_QUOTED };
...
class Lexer {
 ...
 public:
  void init_internal(const char *buffer_start, const char *buffer_end,
                     const char *buffer_ptr);
